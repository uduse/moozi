...
class Model(hk.Module):
    def __init__(self, dim_actions, dim_repr):
        super().__init__()
        self.dim_actions = dim_actions
        self.dim_repr = dim_repr

    def repr_net(self, image):
        net = hk.nets.MLP(output_sizes=[16, 16, self.dim_repr], name='repr')
        return net(image)

    def pred_net(self, hidden_state):
        v_net = hk.nets.MLP(output_sizes=[16, 16, 1], name='pred_v')
        p_net = hk.nets.MLP(output_sizes=[16, 16, self.dim_actions],
                            name='pred_p')
    ...
    def dyna_net(self, hidden_state, action):
        ...
        transition_net = hk.nets.MLP(
            output_sizes=[16, 16, self.dim_repr], name='dyna_trans')
        reward_net = hk.nets.MLP(
            output_sizes=[16, 16, self.dim_repr], name='dyna_reward')
