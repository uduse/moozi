...
class RolloutWorkerWithWeights:
    ...
    root_inf: Callable = field(init=False)
    trans_inf: Callable = field(init=False)
    ...
    def set_model(self, model: mz.nn.NNModel):
        ...
        self.root_inf = jax.jit(model.root_inference, static_argnames="is_training")
        self.trans_inf = jax.jit(model.trans_inference, static_argnames="is_training")
        self.root_inf_unbatched = jax.jit(
            model.root_inference_unbatched, static_argnames="is_training"
        )
        self.trans_inf_unbatched = jax.jit(
            model.trans_inference_unbatched, static_argnames="is_training"
        )
